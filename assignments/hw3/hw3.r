...
# load the data
load("C:/Users/NgaPui/src/stat133/assignments/hw3/WR1500MeterMen.rda")
...
ggplot(data = wr1500m, aes(year, times_sec)) + geom_step()
...
# Your ggplot command:
ggplot(data = wr1500m, aes(year, times_sec)) + geom_step() + geom_segment(aes(x = 1998, xend = 2014, y = wr_1998, yend = wr_1998)) + xlim(1890, 2014)
...
# Your ggplot command
ggplot(data = wr1500m, aes(year, times_sec)) + geom_step() + geom_segment(aes(x = 1998, xend = 2014, y = wr_1998, yend = wr_1998)) + xlim(1890, 2014) + geom_vline(xintercept = 1944, color = "green") + geom_vline(xintercept = 1998, color = "green") + annotate("text", x = 1944, y = wr_1944, label = wr1500m$athlete[(wr1500m$year == 1944)], color = "blue") + annotate("text", x = 1998, y = wr_1998, label = wr1500m$athlete[(wr1500m$year == 1998)], color = "blue")
...
ggplot(data = wr1500m, aes(year, times_sec)) + geom_step() + geom_segment(aes(x = 1998, xend = 2014, y = wr_1998, yend = wr_1998)) + xlim(1890, 2014) + geom_vline(xintercept = 1944, color = "green") + geom_vline(xintercept = 1998, color = "green") + annotate("text", x = 1944, y = wr_1944, label = wr1500m$athlete[(wr1500m$year == 1944)], color = "blue") + annotate("text", x = 1998, y = wr_1998, label = wr1500m$athlete[(wr1500m$year == 1998)], color = "blue") + labs (x = "Year", y = "Time", title = "Men's 1500M Run World Records")
...
load("C:/Users/NgaPui/src/stat133/assignments/hw3/SummerOlympics2012Ctry.rda")
...
# Your ggplot command:
ggplot(data = SO2012Ctry, aes(pop, GDP)) + geom_point()
...
# Your ggplot command
ggplot(data = SO2012Ctry, aes(x = log(pop), y = log(GDP_per_person), size = SO2012Ctry$Total)) + geom_point()
...
ggplot(data = SO2012Ctry, aes(x = log(pop), y = log(GDP_per_person), size = SO2012Ctry$Total)) + geom_point() + labs (x = "Population", y = "GDP Per Person", title = "Olympics Medal Count: GDP Per Person Vs Population") + annotate("text", x = SO2012Ctry$pop[top5], y = SO2012Ctry$GDP_per_person[top5], label = SO2012Ctry$ISO[top5])
...
ggplot() + geom_polygon(data = world_map, aes(x = long, y = lat, group = group), fill = "lightgray", colour = "grey90") + geom_point(data = wonMedal, aes(x = longitude, y = latitude, size = sqrt(Total)), colour = "gold")
...
load("C:/Users/NgaPui/src/stat133/assignments/hw3/London2012ALL_ATHLETES.rda")
...
# Your ggplot commands
ggplot(data = athletes, aes(x = Sport, fill = Sex)) + geom_bar(position = "dodge") + theme(axis.text.x = element_text(angle = 90)) + labs (title = "2012 Olympic Sports Athletes: Men Vs. Women")
...
load("C:/Users/NgaPui/src/stat133/assignments/hw3/rainfallCO.rda")
